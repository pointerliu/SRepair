...
def analyze_ast(ast: vast.Source) -> AnalysisResults:
    ...
    return AnalysisResults(infer.widths, deps.vars, deps.cond_res, infer.var_width_voting)
...
class InferWidths(AstVisitor):
    ...
    def __init__(self):
        ...
        self.var_width_voting = {}

    def set_voting(self, var_name, width, node):
        if var_name not in self.var_width_voting:
            self.var_width_voting[deepcopy(var_name)] = {}
        if width not in self.var_width_voting[var_name]:
            self.var_width_voting[deepcopy(var_name)][width] = []
        self.var_width_voting[deepcopy(var_name)][width].append(node)
    ...
    def expr_width(self, node, env_width) -> int:
        ...
        if isinstance(node, vast.Identifier):
            ...
            width = msb - lsb + 1
            if width < 0:
                node.lsb = vast.IntConst(str(msb))
                node.msb = vast.IntConst(str(lsb))
                width = -width
    ...
    def determine_var_width(self, node):
        ...
        self.set_voting(node.name, self.vars[node.name], node)
    ...
    def generic_visit(self, node):
        if isinstance(node, vast.Variable) or isinstance(node, vast.Input) or isinstance(node, vast.Output):
            ...
            self.generic_visit(node.true_value)
            self.generic_visit(node.false_value)
        elif isinstance(node, vast.Operator):
            lhs_width = self.expr_width(node.left, None)
            rhs_width = self.expr_width(node.right, lhs_width)

            if isinstance(node.left, vast.Identifier):
                self.set_voting(node.left.name, rhs_width, node)
            elif isinstance(node.left, vast.IntConst):
                self.set_voting(node.left, rhs_width, node)

            if isinstance(node.right, vast.Identifier):
                self.set_voting(node.right.name, lhs_width, node)
            elif isinstance(node.right, vast.IntConst):
                self.set_voting(node.right, lhs_width, node)
            ...
            try:
                self.set_voting(node.left.var.name, rhs_width, node.right.var)
            except Exception as e:
                pass

            try:
                if isinstance(node.right.var, vast.IntConst):
                    self.set_voting(node.right.var, lhs_width, node.left.var)
                else:
                    self.set_voting(node.right.var.name, lhs_width, node.left.var)
            except Exception as e:
                pass
    ...
    def visit_Reg(self, node: vast.Reg):
        ...
        if node.name not in self.vars:
            if node.dimensions is not None:
                self.vars[node.name] = VarInfo(node.name, self.widths[node.name], is_vector=True)
            else:
                self.vars[node.name] = VarInfo(node.name, self.widths[node.name])

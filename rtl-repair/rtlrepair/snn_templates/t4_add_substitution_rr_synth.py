...
def t4_add_substitution_rr_synth(ast: vast.Source, analysis: AnalysisResults, bound_arch=[]):
    # df = get_dataflow(ast)
    df = None
    ...
    repl = AddSubstitution(cu, analysis.vars, analysis.widths, df, assignment, blocking_assignment,
...
class AddSubstitution(SNNTemplate):
    ...
    def visit_NonblockingSubstitution(self, node: vast.NonblockingSubstitution):
        ...
        # else:
        snn_expr = self.build_guard(node.right.var, candi_vars, bv=self.get_width(node))

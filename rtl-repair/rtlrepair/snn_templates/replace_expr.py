...
from rtlrepair.snn_templates.utils import get_dataflow, analysis_if_body
...
def replace_expr(ast: vast.Source, analysis: AnalysisResults):
    df = get_dataflow(ast)
    namespace = Namespace(ast)
    repl = ExprReplacer(analysis.vars, analysis.widths, df)
...
class ExprReplacer(RepairTemplate):
    def __init__(self, vars: dict[str, VarInfo], widths: dict[vast.Node, int], dataflow: Dict[str, Set[str]]):
    ...
    def _check_comb_loop(self, dest, src):
        df_next_bk = self.df_next.copy()
        if src not in df_next_bk:
            df_next_bk[src] = set()
        df_next_bk[src].add(dest)
        ...
        def top_dfs(s):
            if s not in visited: return True
            if visited[s]:
                return False
            visited[s] = True
            for nxt in df_next_bk[k]:
                res = top_dfs(nxt)
                if not res:
                    return False
            return True
    ...
    def visit_NonblockingSubstitution(self, node: vast.NonblockingSubstitution):
        ...
        if isinstance(node.left.var, vast.Partselect):
            self._bv = int(node.left.var.msb.value) - int(node.left.var.lsb.value) + 1
        else:
            try:
                self._bv = self.vars[node.left.var.name].width
    ...
    def visit_BlockingSubstitution(self, node: vast.BlockingSubstitution):
        # in_vars = [
        #     vast.Identifier(name) for name in self.vars if
        #     self._check_comb_loop(node.left.var.name, name) and
        #     self.vars[name].width == self.vars[node.left.var.name].width
        # ]
    ...
    def visit_CaseStatement(self, node: vast.CaseStatement):
        ...
        # synthesis new case arm
        s_case = deepcopy(node.caselist[0])
        s_case.cond = (self.make_change(self.build_snn(nodes_num=[1], in_vars=[]), s_case.cond[0]),)
    ...
    #
    #     self._cond_ops = True
    #     cond = self.build_snn(nodes_num=nodes_num, in_vars=in_vars)
    ...
    #         false_statement=node.false_statement,
    #     )
    #
    #     return node

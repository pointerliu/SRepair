...
def load_benchmark(project_path: Path, bug_name: str, testbench_name: str, is_mut: bool = False) -> Benchmark:
    project = benchmarks.load_project(project_path, is_mut)
...
def load_config(filename: Path, working_dir: Path, benchmark: Benchmark, sim: str, simulator_compile_timeout: float, seed: str, is_mut: bool=False) -> Config:
...
def run(conf: Config, repair_file: Path = None, verbose: bool = False) -> Path:
    ...
    output = conf.working_dir / benchmark.design.directory.name / benchmark.testbench.output
    if output.exists():
        os.remove(output)
...
def calc_candidate_fitness(conf: Config, fileName: Path):
    ...
    if conf.verbose:
        print(f"Running simulation with simulator {conf.simulator}")

    if conf.simulator == 'verilator':
        benchmarks.copy_whole_bug_dir(benchmark.design.directory, conf.working_dir, exist_ok=False)
        benchmarks.copy_whole_bug_dir(benchmark.design.directory.parent / 'common', conf.working_dir, exist_ok=True)

        # cmd = f"sed 's/{}/{}/g' source_bug_{}.txt > sources_patch.txt"
        relative_path = benchmark.bug.buggy.relative_to(benchmark.design.directory)
        if not conf.is_mut:
            assert (conf.working_dir / benchmark.design.directory.name / f'sources_bug_{benchmark.bug.name}.txt').exists()
            cmd = ["sed", f"'s|{relative_path}|{fileName}|g'", f"sources_bug_{benchmark.bug.name}.txt", ">", "sources_patch.txt"]
        else:
            assert (conf.working_dir / benchmark.design.directory.name / f'sources_mut_{benchmark.bug.name}.txt').exists()
            cmd = ["sed", f"'s|{relative_path}|{fileName}|g'", f"sources_mut_{benchmark.bug.name}.txt", ">", "sources_patch.txt"]

        cmd = ' '.join(cmd)
        ret = subprocess.run(cmd, cwd=conf.working_dir / benchmark.design.directory.name, shell=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
        print(ret)
...
def main():
    ...
    optparser.add_argument("--is-mut", action="store_true", default=False)
    ...
    benchmark = load_benchmark(Path(args.project), args.bug, args.testbench, args.is_mut)
    conf = load_config(Path(args.config), Path(args.working_dir), benchmark, args.simulator,
                       args.simulator_compile_timeout, seed=SEED, is_mut=args.is_mut)

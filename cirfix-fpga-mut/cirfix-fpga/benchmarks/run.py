...
def run(working_dir: Path, sim: str, files: list, conf: RunConf) -> bool:
    if sim == 'vcs':
        ...
        return run_with_verilator(working_dir, files, conf)
    else:
...
def run_with_verilator(working_dir: Path, files: list, conf: RunConf) -> bool:
    cmd = ['make clean &&', 'make', 'SIM_SOURCES=sources_patch']
    working_dir = working_dir / conf.include_dir.name
    _print(conf, " ".join(str(c) for c in cmd))
    stdout, stderr = _conf_streams(conf)
    # while iverilog generally does not timeout, we add the timeout here for feature parity with the VCS version
    cmd = ' '.join(cmd)
    try:
        _flush_log(conf)
        r = subprocess.run(cmd, cwd=working_dir, shell=True, check=False, stdout=stdout, stderr=stderr,
                           timeout=conf.compile_timeout)
        # r = subprocess.run(cmd, cwd=working_dir, shell=True)
        compiled_successfully = r.returncode == 0
    except subprocess.TimeoutExpired:
        compiled_successfully = False
    _flush_log(conf)
    ...
    if compiled_successfully:
        try:
            _print(conf, './sources_patch_test')
            _flush_log(conf)
            r = subprocess.run(f'./sources_patch_test', cwd=working_dir, shell=True, timeout=conf.timeout, stdout=stdout,
                               stderr=stderr)
            success = r.returncode == 0
        except subprocess.TimeoutExpired:
            success = False  # failed
        _flush_log(conf)
        os.remove(os.path.join(working_dir, 'sources_patch_test'))
        return success
    else:
        return False  # failed to compile
...
_vcs_flags = ["-sverilog", "-full64", "-LDFLAGS -Wl,--no-as-needed"]
